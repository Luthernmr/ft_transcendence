...
export class AuthController {
    ...
    try {
      ...
      if (!user)
        throw new BadRequestException(
          'Impossible to create user with this informations',
        );
    } catch (error) {
      return error;
    }
  ...
  async login(
    ...
    try {
      ...
      if (user.isOnline) throw new BadRequestException('Already connected');
  ...
  async verifyJWT(
   ...
   const  tes = await this.authService.verifyJWT(data?.jwt);
		if(!tes)
			throw new UnauthorizedException('failed to load jwt')
    } catch (error) {
		throw new UnauthorizedException('failed to load jwt')
    }
  }
  ...
  @Post('turn-on')
  @UseGuards(LocalAuthGuard)
  async turnOnTwoFA(@Req() request: Request, @Body() data: any) {
    try {
      ...
      const user = await this.authService.getUserCookie(request);
      if (!user.id) throw new BadRequestException('failed to load user');
