...
export class UserController {
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        ...
        filename(req, file, callback) {
          ...
          const newFileName =
            name.substring(0, 10).split(' ').join('_') + '_' + Date.now();
  ...
  async addAvatar(
    ...
    try {
      if (!file) throw new BadRequestException('Please enter valid file');
      const test = filetype(fs.readFileSync(file.path));
      if (test.length) {
        if (
          !(
            (test[0].mime == 'image/png' ||
              test[0].mime == 'image/jpeg' ||
              test[0].mime == 'image/jpg' ||
              test[0].mime == 'image/gif') &&
            file.size < 5000000
          )
        ) {
